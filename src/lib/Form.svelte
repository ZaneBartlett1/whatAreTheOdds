<script lang="ts">
  ...
  function calculate() {
    ...
    if (selectedSex === "M") {
      [sizeFrom, sizeTo] = setDefaultRangeValues(sizeData, sizeFrom, sizeTo);
    }
    ...
    if (heightFrom !== "" && heightTo !== "") {
      ...
      for (let heightGroup of Object.keys(heightData)) {
        ...
        if (inRange) heightLikelihood += heightData[heightGroup][selectedSex];
    ...
    let state = selectedCity[0].split(",")[0].trim();
    if (obesityStatus === "notObese") {
      weightLikelihood = weightData[state] / 100;
    } else if (obesityStatus === "Obese") {
      weightLikelihood = (100 - weightData[state]) / 100;
    }
    // if obesityStatus is "either", weightLikelihood remains 1
  ...
  function getToOptions(dataObject, fromVar) {
    return fromVar === ""
      ? []
      : Object.keys(dataObject).slice(Object.keys(dataObject).indexOf(fromVar));
  }
...
<div class="form-option">
  <label for="multi-select-box-city">
    <span class="label-text">
      <sup>
        <a href={populationDataSource} target="_blank" rel="noopener"> [4] </a>
      </sup>
...
<div class="form-option">
  <label for="height-start-select">
    <span class="label-text">
      <sup>
        <a href={heightDataSource} target="_blank" rel="noopener"> [2] </a>
      </sup>
      Height From:
    </span>
  </label>
  <div class="form-option-content">
    <select id="height-start-select" bind:value={heightFrom}>
      ...
      {#each Object.keys(heightData) as heightOption}
        <option value={heightOption}>{heightOption.split("-")[0]}</option>
      {/each}
    </select>
...
<div class="form-option">
  <label for="income-start-select">
    <span class="label-text">
      <sup>
        <a href={incomeDataSource} target="_blank" rel="noopener"> [3] </a>
      </sup>
      Income From:
    </span>
  </label>
  <div class="form-option-content">
    <select id="income-start-select" bind:value={incomeFrom}>
      ...
      {#each Object.keys(incomeData) as incomeOption}
        <option value={incomeOption}>{incomeOption.split("-")[0]}</option>
      {/each}
    </select>
...
{#if selectedSex === "M"}
  <div class="form-option">
    <label for="penis-start-legnth-select">
      <span class="label-text">
        <sup>
          <a href={sizeDataSource} target="_blank" rel="noopener">[8]</a>
        </sup>
        Penis Length From:
      </span>
    </label>
    <div class="form-option-content">
      <select id="penis-start-legnth-select" bind:value={sizeFrom}>
        <option disabled selected value="">-- select start size --</option>
        {#each Object.keys(sizeData) as sizeOption}
          <option value={sizeOption}>{sizeOption.split("-")[0]}</option>
        {/each}
      </select>
...
<div class="form-option">
  <label for="obesity-dropdown">
    <span class="label-text">
      <sup>
        <a href={weightDataSource} target="_blank" rel="noopener"> [6] </a>
      </sup>
      Obesity:
    </span>
  </label>
  <div class="form-option-content">
    <select bind:value={obesityStatus} id="obesity-dropdown">
      <option value="either">Either</option>
      <option value="notObese">Not Obese</option>
...
<div class="form-option">
  <label for="multi-select-box-ethnicity">
    <span class="label-text">
      <sup>
        <a href={ethnicityDataSource} target="_blank" rel="noopener">[5]</a>
      </sup>
      Ethnicity:
    </span>
  </label>
...
<div class="form-option">
  <label for="multi-select-box-affiliations">
    <span class="label-text">
      <sup>
        <a href={affiliationDataSource} target="_blank" rel="noopener"> [7] </a>
      </sup>
